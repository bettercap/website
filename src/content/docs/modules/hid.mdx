---
title: HID on 2.4Ghz
description: This module, which is a port of Bastille's mousejack attack, performs scanning and frames injection for HID devices on the 2.4Ghz spectrum, using Nordic Semiconductor nRF24LU1+ based USB dongles and Bastille's RFStorm firmware.
sidebar:
  order: 4
---

This module is a port of [Bastille's mousejack attack](https://www.mousejack.com/).
It performs **scanning** and **frames injection** for HID devices on the **2.4Ghz spectrum**.
Uses Nordic Semiconductor nRF24LU1+ based USB dongles with [RFStorm firmware](https://github.com/BastilleResearch/nrf-research-firmware).

The module works with **RFStorm-supported devices**:
...
Start **sniffing** a specific `ADDRESS` to collect payloads.
Use `clear` to stop collecting.

### `hid.inject ADDRESS LAYOUT FILENAME`

Parse the **DuckyScript** `FILENAME` and inject it as HID frames.
Spoofs the device at `ADDRESS` using the `LAYOUT` keyboard mapping.

**Available layouts**: `BE BR CA CH DE DK ES FI FR GB HR IT NO PT RU SI SV TR US`

:::note
The `hid.inject` command does not require the HID device to be visible via `hid.show`.
If you know the dongle address, set `hid.force.type` to `logitech`, `amazon`, or `microsoft`.
Then run the injection "blindly".
:::
...
Enable **HID discovery** and use `ticker` to display detected devices.
Wait for device `32:26:9f:a4:08` and inject `ducky.txt` with US keyboard layout:
...
Send `ducky.txt` to dongle `32:26:9f:a4:08` forcing type `logitech`.
No need to wait for a connected device to be visible:
...
<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/TdPRYWkYarM"
  frameborder="0"
