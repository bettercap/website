...
This module is [enabled by default](/usage/interactive_session/#-autostart-modules).
It reports **events** (logs, new hosts, etc) from other modules.
...
Use placeholders in COMMANDS to access event payload fields.
Supports [XPath queries on JSON](https://github.com/antchfx/xpath) between brackets.
...
When a new **WiFi client** is found, launch a [deauth attack](/modules/wifi/#wifi-deauth-bssid).
When a new **access point** is found, try to associate:
